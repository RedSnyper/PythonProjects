...
for i in range(0, years+1):
    ...
    sl_dep.append((book_val_at_beg[i]-salvage)/(years-i+1))
    if db_dep[i] > sl_dep[i]:
        switch_over_dec.append('>')
        selected_dep.append(db_dep[i])
        book_val_at_end.append(book_val_at_beg[i] - selected_dep[i])
    else:
        switch_over_dec.append('<')
        selected_dep.append(sl_dep[i])
        book_val_at_end.append(book_val_at_beg[i] - selected_dep[i])

print('adjust the table data to the table column in copy')
print('book value at beginning \t db depreciation \t switchover decision \t sl dep \t selected dep \t book value at end')
for i in zip(book_val_at_beg, db_dep, switch_over_dec,sl_dep,selected_dep,book_val_at_end):
    print(i)
